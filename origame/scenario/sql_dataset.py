...
It is not the dataset directly returned from the sql query. Instead, it supports data manipulation in a pythonic
...
class SqlDataSet:
    ...
    def __init__(self, table_name: str, sql_statement: str,
    ...
    def __getitem__(self, index: Either[int, str, RangeDef]) -> Either[Any, Record, Decl.SqlDataSet]:
        ...
        new_sql_data_set = SqlDataSet(self.__table_name, self.__sql_statement,
    ...
    def __repr__(self):
        """The repr of the underlying raw data received from SQL"""
        if self.__data is None:
            self.__get_data()
        assert self.__data is not None
